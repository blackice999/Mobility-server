...
$users_id = 2; // you can your integerate authentication module here to get logged in member
    ...
    switch ($_GET["action"]) {
        case "add":
            if (! empty($_POST["quantity"])) {
                ...
                $productResult = $shoppingCart->getProductByCode($_GET["categories_id"]);
                
                $cartResult = $shoppingCart->getCartItemByProduct($productsResult[0]["id"], $users_id);
                ...
                if (! empty($cartResult)) {
                    ...
                    // Add to cart table
                    $shoppingCart->addToCart($productsResult[0]["id"], $_POST["quantity"], $users_id);
                }
            }
            break;
        case "remove":
        ...
        case "empty":
            // Empty cart
            $shoppingCart->emptyCart($users_id);
            break;
    }
}
?>
...
<BODY>
<?php
$cartItem = $shoppingCart->getMemberCartItem($users_id);
